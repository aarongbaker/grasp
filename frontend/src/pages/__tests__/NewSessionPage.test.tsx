...
describe('NewSessionPage', () => {
  ...
  it('renders the planner lane with inline reference controls and guidance links', async () => {
    ...
    expect(
      screen.getByText(/No owned reference needed unless you want the planner anchored/i),
    ).toBeInTheDocument();
    ...
    expect(screen.getByText(/Resolve an owned recipe title/i)).toBeInTheDocument();
  });
  ...
  it('surfaces planner resolution API failures inline while preserving the form', async () => {
    ...
    expect(await screen.findByText('Resolution unavailable')).toBeInTheDocument();
